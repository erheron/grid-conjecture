...
def add_grid_constraints(solver, x):
    for i in range(5):
        if i == 0 or i == 4:
            solver.Add(7*sum(x[i,j] for j in range(5)) == 2)
        else:
            solver.Add(7*sum(x[i,j] for j in range(5)) == 1)
    for j in range(5):
        if j == 0 or j == 4:
            solver.Add(7*sum(x[i,j] for i in range(5)) == 1)
        else:
            solver.Add(7*sum(x[i,j] for i in range(5)) == 1)
...
def create_variables_with_basic_constraints(solver, x):
    ...
    for i in range(5):
        for j in range(5):
            x[(i,j)] = solver.NumVar(0, solver.infinity(), 'x_{}_{}'.format(i,j))
...
def add_subset_constraints(solver, subset, x):
    ...
    for l in range(len(subset)//4):
        ...
        solver.Add(7*sum(x[i,j] for i,j in rect_coords) >= 2*1 + 7*x['delta'])
        # 7 * x[i,j] >= 2*n+ 7*delta
